...
function CreateOrder() {
  ...
  const {
    ...
    adress,
    error: errorAdress,
  } = useSelector((state) => state.user); // to get entire user from redux state //

  const isLoadingAdress = adressStatus === "loading";

  const navigation = useNavigation(); // gets the current status like loading , idle it is inbuild  //
  const isSubmitting = navigation.state === "submitting";

  const formErrors = useActionData(); // to get data from errors object //
  ...
  // the Form below is Form from react-router //
  return (
...
export async function action({ request }) {
  ...
  store.dispatch(clearCart()); // use this technique very rarely using store and dispatch function in normal function in  action  // dont overuse it it deactivates many performance optimization in redux //
  // after placing order it clears the cart //
